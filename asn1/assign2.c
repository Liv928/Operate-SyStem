...
int main(){
	...
	if(pid1>0&&pid2>0){//parent
		...
		printf("\n Parent process(PID %d) is waiting for child_1(PID%d) to complete before creating child_2.",i,pid1);
		wait(NULL);
		printf("\n Parent process(PID%d) created child_2(PID%d).",i,pid2);
	...
	if(pid1>0&&pid2==0){//child2
		...
		//printf("\n Child_2 (PID %d) is calling an external program external_program.out and leaving child_2… ",i);
