...
{   
    ...
    while(p!=NULL){    
        ...
        if(strstr(p,"t")){
            ...
            p=strtok(NULL, " ");//account 1
            a_one[0]=p[1];
            a=atoi(a_one);
              
            p=strtok(NULL, " ");//account 2
            a_two[0]=p[1];
            b=atoi(a_two);
...
int main(){
    ...
    int i,x, err_thread, err_thread_cli;
    ...
    for (i = 0; i< dep_num; i++){
        err_thread = pthread_create(&threads[i], NULL, &depo_run, NULL); 
        if (err_thread != 0)
    ...
    for (x = 0; x< cli_num; x++){
        err_thread_cli = pthread_create(&threads_cli[x], NULL, &thread_run, NULL); 
        if (err_thread_cli != 0)
